...
class SearchSecurityScorecard:
    ...
    def _extract_data(self, data: dict) -> None:
        ...
        if isinstance(data.get('ips'), list):
            ips = [str(ip) for ifconfig in data.get('ips', []) if isinstance(ip, str | int)]
        elif isinstance(data.get('ip_addresses'), list):
            ips = [str(ip) for ifconfig in data.get('ip_addresses', []) if isinstance(ip, str | int)]
        elif isinstance(data.get('associated_ips'), list):
            ips = [str(ip) for ifconfig in data.get('associated_ips', []) if isinstance(ip, str | int)]
        if ips:

...
class AdditionalAPIs:
    ...
    async def _process_shodan(self, proxy: bool = False):
        ...
        try:
            ...
            try:
                ifconfig = socket.gethostbyname(self.domain)
            ...
            for host in self.hosts:
                if ':' in host:
                    # Extract IP from host:ifconfig format
                    parts = host.split(':')
            ...
            for ifconfig in ips_to_search:
                try:
                    print(f'\tSearching Shodan for {ip}')
                    shodan_result = await self.shodan.search_ip(ip)

                    if ifconfig in shodan_result and isinstance(shodan_result[ip], dict):
                        self.shodan_data[ip] = shodan_result[ip]
                    elif ifconfig in shodan_result and isinstance(shodan_result[ip], str):

...
async def start(rest_args: argparse.Namespace | None = None):
    ...
    async def store(
        ...
        if store_host:
            ...
            if source != 'hackertarget' and source != 'pentesttools' and source != 'rapiddns':
                # If a source is inside this conditional, it means the hosts returned must be resolved to obtain ifconfig
                        ...
                        class ShodanWrapper:
                            ...
                            async def process(self, use_proxy: bool = False):
                                ...
                                try:
                                    ...
                                    ifconfig = socket.gethostbyname(self.word)
                                    print(f'\tSearching Shodan for {ip}')
                                    result = await self.shodan.search_ip(ip)
                                    if ifconfig in result and isinstance(result[ip], dict):
                                    ...
                                    elif ifconfig in result and isinstance(result[ip], str):
    ...
    if rest_args is not None and len(rest_filename) == 0 and rest_args.dns_brute is False:
        ...
        return_ips.extend([str(ip) for ifconfig in sorted([netaddr.IPAddress(ip.strip()) for ifconfig in set(all_ip)])])
    ...
    if filename != '':
        ...
        try:
            ...
            async with await anyio.open_file(filename, 'w+') as file:
                ...
                for x in full:
                    host, ifconfig = x.split(':', 1) if ':' in x else (x, '')
                    if ifconfig and len(ip) > 3:
                        await file.write(
